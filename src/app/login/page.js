...
export default async function LoginPage({ searchParams }) {
  ...
  return (
    <div className="flex-1 flex flex-col w-full px-8 sm:max-w-md justify-center gap-2 mt-20 mx-auto min-h-[70vh]">
      ...
      <form className="animate-in flex-1 flex flex-col w-full justify-center gap-2 text-textmain" action={login}>
        <div className="text-center mb-8">
          <h1 className="text-3xl font-serif mb-2">Welcome Back</h1>
          <p className="text-textmuted font-light">Sign in to manage your appointments</p>
        </div>
        
        <label className="text-sm font-medium mt-4 uppercase tracking-wider text-textmuted" htmlFor="email">
          Email
        </label>
        <input
